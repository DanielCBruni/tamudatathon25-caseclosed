...
# MY CODE: IMPORTING MODEL FROM HELPER FILES #####################################

import torch
from model_bc import TheMover, preprocess, action_from_logits

MODEL_PATH = "data/bc_model.pth"
MODEL_DEVICE = "cpu"
...
def send_move():
    ...
    # MY CODE: RUNNING MODEL AND MAKING DECISIONS ##############################################

    # Build model input from current board state
    board = state["board"]
    my_trail = state["agent1_trail"] if player_number == 1 else state["agent2_trail"]
    enemy_trail = state["agent2_trail"] if player_number == 1 else state["agent1_trail"]

    # Convert to model input tensor
    inp = preprocess(board, my_trail, enemy_trail)

    # Run model inference
    with torch.no_grad():
        logits = model(inp)

    # Convert logits → Direction Enum
    action = action_from_logits(logits[0])

    move = action.name   # "UP", "DOWN", etc.
